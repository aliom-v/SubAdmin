...
- 保存策略后增加成功提示，并根据缓存模式给出“已自动刷新输出缓存”或“可手动执行全量同步”的说明。
- preview 增加“与当前草稿一致 / 已过期”的状态提示，并在草稿变化后要求重新预览。
- 离开系统设置页、刷新页面或退出登录前，未保存策略草稿会触发提醒。
- `priority_override` 模式增加丢弃低优先级冲突节点的风险提示；若当前 preview 有效，会直接显示预计丢弃数量。
- 当手动节点或上游原始缓存内容发生变化时，旧 preview 会被清空，避免误把旧结果当成当前结果。
