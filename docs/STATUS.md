...
- 已完成最新 Phase 4 回归：`make acceptance` 全量通过（2026-03-08 23:20 +08:00），已形成包含策略接口、策略日志断言与临时数据卷隔离的最新本地基线。
- 已补齐单独的 Phase 4 回归记录与指标监控文档。
- 已补齐 Phase 4 压测抽检脚本与 CI 证据产物上传能力。
- 已完成高级同步策略 V1 闭环：新增策略配置、preview API、管理后台配置入口、输出聚合链路、策略指标与后端接口级测试。
- 已完成前端控制台首轮拆分：`web/src/App.jsx` 已拆为登录、顶部导航、上游、节点、设置、备份、日志等独立组件。
- 已完成策略 UX 首轮反馈：保存成功提示、preview 过期标记、未保存变更提醒与 `priority_override` 风险提示。
- 当前主要待办为按需执行真实压测抽检、在 CI 首次远端运行后补充远端证据产物，并继续推进更细粒度策略能力。
...
| 高级同步策略 | 已完成（V1） | 已支持 `/api/strategy`、`/api/strategy/preview`、管理后台配置与预览；已补齐策略指标、接口级测试、基础 UX 提示与最新本地回归基线；更细粒度策略仍为后续增强 |
...
- 高级同步策略 V1 已具备 API、管理后台配置、独立运维手册、策略指标、接口级测试与最新本地回归基线；更细粒度策略仍待后续增强。
...
3. 已完成高级同步策略 V1 闭环与首轮策略 UX 反馈，下一步按需推进远端验证证据补齐与更细粒度策略能力。
...
3. 已完成：运行 `make acceptance`（2026-03-08 23:20 +08:00），形成包含策略接口、策略日志断言与临时数据卷隔离的最新本地通过基线。
...
- 完成高级同步策略观测补齐：新增策略 preview/apply/冲突组/丢弃节点指标，并同步更新相关文档。
- 完成 `/api/strategy` 后端 HTTP 层集成测试，覆盖默认配置、非法模式、preview/apply 持久化与缓存模式刷新。
- 完成 `scripts/phase4_acceptance.sh` 隔离增强：本地验收时为 `api` 注入临时 Docker 数据卷，避免历史 `data/` 状态污染基线。
- 完成前端控制台首轮组件化拆分：`web/src/App.jsx` 收口为状态与动作编排层，面板视图拆分为独立组件。
- 完成策略 UX 首轮反馈：新增保存成功提示、预览过期标记、未保存变更提醒与 `priority_override` 风险提示。
- 完成最近优化摘要文档整理：新增 `docs/RECENT_OPTIMIZATIONS.md` 统一沉淀本轮变更与下一步建议。
