...
- 当前主要待办为按需执行真实压测抽检、在 CI 首次远端运行后补充远端证据产物，并继续优化策略 UX。
...
| 高级同步策略 | 已完成（V1） | 已支持 `/api/strategy`、`/api/strategy/preview`、管理后台配置与预览；已补齐策略指标、接口级测试与最新本地回归基线；更细粒度策略仍为后续增强 |
...
3. 已完成高级同步策略 V1 闭环，下一步按需推进策略 UX 打磨与更细粒度策略能力。
